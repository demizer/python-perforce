...
ConnectionStatus = namedtuple(
    "ConnectionStatus", "OK, OFFLINE, NO_AUTH, INVALID_CLIENT"
)(*range(4))
...
def split_ls(func):
    """Decorator to split files into manageable chunks as not to exceed the windows cmd
    limit
...
class Connection(object):
    """Connection to perforce that handles all of the communication with the perforce
    server"""
    ...
    def __init__(
        ...
        if self._port is None:
            raise errors.ConnectionError(
                "Perforce host could not be found, please set P4PORT or provide the"
                "hostname and port"
            )

        if self._user is None:
            raise errors.ConnectionError(
                "No user could be found, please set P4USER or provide the user"
            )

    def __repr__(self):
        return "<Connection: {0}, {1}, {2}>".format(
            self._port, str(self._client), self._user
        )
        ...
        if marshal_output:
            try:
                while True:
                    ...
                    if (
                        record.get(b"code", "") == b"error"
                        and record[b"severity"] >= self._level
                    ):
                        proc.stdin.close()
                        proc.stdout.close()
    ...
    def ls(self, files, silent=True, exclude_deleted=False):
        ...
        :type files: list
        :param silent: No exceptions are raised for invalid files or files not under the
        client
        :type silent: bool
            ...
            if isinstance(description, six.integer_types):
                ...
                for cl in pending:
                    if cl["desc"].strip() == description.strip():
                        LOGGER.debug("Changelist found: %s", cl["change"])
    ...
    def canAdd(self, filename):
        ...
        LOGGER.warn("Unable to add %s: %s", filename, result["data"])
...
class Changelist(PerforceObject):
    """A Changelist is a collection of files that will be submitted as a single entry with
    ...
    def query(self, files=True):
        ...
        if self._change:
            ...
            self._p4dict = {
                camel_case(k): v
                for k, v in six.iteritems(self._connection.run(["change", "-o", cl])[0])
            }
    ...
    def append(self, rev):
        """Adds a :py:class:Revision to this changelist and adds or checks it out if
        needed
    ...
    def delete(self):
        """Reverts all files in this changelist then deletes the changelist from
        perforce"""
        try:
            self.revert()
...
class Revision(PerforceObject):
    ...
    def __len__(self):
        if "fileSize" not in self._p4dict:
            self._p4dict = self._connection.run(
                ["fstat", "-m", "1", "-Ol", self.depotFile]
            )[0]
    ...
    def query(self):
        ...
        self._p4dict = self._connection.run(
            ["fstat", "-m", "1", self._p4dict["depotFile"]]
        )[0]
        self._head = HeadRevision(self._p4dict)
    ...
    def edit(self, changelist=0):
        ...
        if int(changelist):
            self._connection.run(
                [command, "-c", str(changelist.change), self.depotFile]
            )
        else:
